...
def create_cooccurrence_matrix(answers, vocab, ngram_range):
    """
    Creates bigram co-occurrence matrices for a list of answers.
    Each answer is represented as a matrix where rows and columns correspond to vocabulary words.
    Utilizes CuPy for GPU acceleration.
    """
    from sklearn.feature_extraction.text import CountVectorizer
    vectorizer = CountVectorizer(ngram_range=ngram_range, vocabulary=vocab)
    X = vectorizer.transform(answers)
    
    cooc_matrices = []
    for answer in answers:
        # Initialize a zero matrix on GPU
        cooc_matrix = cp.zeros((len(vocab), len(vocab)), dtype=cp.float32)
        ...
        for bigram in bigrams:
            if bigram[0] in vocab and bigram[1] in vocab:
                ...
                j = vocab[bigram[1]]
                cooc_matrix[i][j] += 1
        cooc_matrices.append(cooc_matrix)
    return cp.array(cooc_matrices)
...
# def pad_lists(dataset):
#     """
#     Ensures all entries in the dataset have 'question', 'label', and 'answer' keys.
#     """
#     required_keys = ['question', 'label', 'answer']
#     padded_dataset = []
#     for entry in dataset:
#         padded_entry = {key: entry.get(key, None) for key in required_keys}
#         padded_dataset.append(padded_entry)
...
def main():
    ...
    # Initialize separate dictionaries for real and halu answers
    real_dataset = {
        "question": [],
        "label": [],  # 1 for real answers
        "answer_text": [],
        "latent_vector": []
    }
    ...
    # Process all answers collectively in batches
    print("Processing all answers in batches...")

    # Extract all real and hallucinated answers
    real_answers = df[df['label'] == 1]['answer'].dropna().tolist()
    halu_answers = df[df['label'] == 0]['answer'].dropna().tolist()

    # Create a unified vocabulary for the entire dataset
    global_vocab = create_local_vocab(real_answers, halu_answers, 1)

    # Split real_answers and halu_answers into batches
    real_batches = list(split_into_batches(real_answers, batch_size))
    halu_batches = list(split_into_batches(halu_answers, batch_size))
    ...
    # Perform SVD on real_answers in batches using PyTorch on GPU
    print(f"Processing {len(real_batches)} batches of real answers...")
    for batch_idx, real_batch in enumerate(real_batches):
        print(f"Processing real answer batch {batch_idx + 1}/{len(real_batches)}...")
        X_real = create_cooccurrence_matrix(real_batch, global_vocab, ngram_range)  # CuPy array on GPU
        
        for i, batch_real in enumerate(X_real):
            if batch_real.shape[0] > 0 and batch_real.shape[1] > 1:
                try:
                    # Convert CuPy array to NumPy array, then to PyTorch tensor on GPU
                    tensor_real = torch.from_numpy(cp.asnumpy(batch_real)).float().to(device)
                    
                    # Perform SVD using PyTorch
                    U, S, V = torch.svd(tensor_real)
                    
                    # Take only the top n_svd_components
                    U_top = U[:, :n_svd_components]
                    S_top = S[:n_svd_components]
                    latent_vectors = torch.mm(U_top, torch.diag(S_top)).cpu().numpy()  # Move back to CPU
                    
                    latent_vectors_real.append(latent_vectors.tolist())
                except Exception as e:
                    print(f"Error performing SVD on real answers batch {i + 1} of batch {batch_idx + 1}: {e}")
                    latent_vectors_real.append(None)  # Placeholder for failed SVD
            else:
                print(f"Skipping real answers batch {i + 1} of batch {batch_idx + 1} due to insufficient data for SVD.")
                latent_vectors_real.append(None)  # Placeholder for skipped batch

    # Perform SVD on halu_answers in batches using PyTorch on GPU
    print(f"Processing {len(halu_batches)} batches of hallucinated answers...")
    for batch_idx, halu_batch in enumerate(halu_batches):
        print(f"Processing halu answer batch {batch_idx + 1}/{len(halu_batches)}...")
        X_halu = create_cooccurrence_matrix(halu_batch, global_vocab, ngram_range)  # CuPy array on GPU
        
        for i, batch_halu in enumerate(X_halu):
            if batch_halu.shape[0] > 0 and batch_halu.shape[1] > 1:
                try:
                    # Convert CuPy array to NumPy array, then to PyTorch tensor on GPU
                    tensor_halu = torch.from_numpy(cp.asnumpy(batch_halu)).float().to(device)
                    
                    # Perform SVD using PyTorch
                    U, S, V = torch.svd(tensor_halu)
                    ...
                    print(f"Error performing SVD on halu answers batch {i + 1} of batch {batch_idx + 1}: {e}")
                    latent_vectors_halu.append(None)  # Placeholder for failed SVD
            else:
                print(f"Skipping halu answers batch {i + 1} of batch {batch_idx + 1} due to insufficient data for SVD.")
                latent_vectors_halu.append(None)  # Placeholder for skipped batch

    # Populate real_dataset
    real_questions = df[df['label'] == 1]['question'].tolist()
    real_answers_text = df[df['label'] == 1]['answer'].dropna().tolist()
    for question, answer_text, latent_vector in zip(real_questions, real_answers_text, latent_vectors_real):
        if latent_vector is not None:
            real_dataset['question'].append(question)
            real_dataset['label'].append(1)
            real_dataset['answer_text'].append(answer_text)
            real_dataset['latent_vector'].append(latent_vector)
        else:
            print(f"Skipping real answer '{answer_text}' due to missing latent vector.")

    # Populate halu_dataset
    halu_questions = df[df['label'] == 0]['question'].tolist()
    halu_answers_text = df[df['label'] == 0]['answer'].dropna().tolist()
    for question, answer_text, latent_vector in zip(halu_questions, halu_answers_text, latent_vectors_halu):
        if latent_vector is not None:
            halu_dataset['question'].append(question)
            halu_dataset['label'].append(0)
            halu_dataset['answer_text'].append(answer_text)
            halu_dataset['latent_vector'].append(latent_vector)
        else:
            print(f"Skipping halu answer '{answer_text}' due to missing latent vector.")

    # Create DataFrames
    real_df = pd.DataFrame(real_dataset)
    halu_df = pd.DataFrame(halu_dataset)

    # Concatenate into a single DataFrame
    latent_df = pd.concat([real_df, halu_df], ignore_index=True)
    ...
    # Compute Procrustes distances and ROUGE scores for real_right and halu_halu
    for idx, row in latent_df.iterrows():
        ...
        if label == 1:
            # Real answers
            real_latents = latent_df[(latent_df['question'] == question) & (latent_df['label'] == 1)]['latent_vector'].tolist()
            real_texts = latent_df[(latent_df['question'] == question) & (latent_df['label'] == 1)]['answer_text'].tolist()
            
            # Compute right_answer ↔ right_answer distances
            for i, j in itertools.combinations(range(len(real_latents)), 2):
                # Calculate ROUGE score
                rouge_score = scorer.score(real_texts[i], real_texts[j])
    
                ra1 = np.array(real_latents[i])
                ra2 = np.array(real_latents[j])
                if np.array_equal(ra1, ra2):
                    continue  # Skip identical vectors
                try:
                    _, _, disparity = procrustes(ra1.reshape(-1, 1), ra2.reshape(-1, 1))
                    procrustes_distances["right_right"].append({
                        "question": question,
                        "pair_indices": (i, j),
                        "distance": disparity,
            ...
            # Halus
            halu_latents = latent_df[(latent_df['question'] == question) & (latent_df['label'] == 0)]['latent_vector'].tolist()
            halu_texts = latent_df[(latent_df['question'] == question) & (latent_df['label'] == 0)]['answer_text'].tolist()
            
            # Compute halu_answer ↔ halu_answer distances
            for i, j in itertools.combinations(range(len(halu_latents)), 2):
                # Calculate ROUGE score
                rouge_score = scorer.score(halu_texts[i], halu_texts[j])
    
                ha1 = np.array(halu_latents[i])
                ha2 = np.array(halu_latents[j])
                if np.array_equal(ha1, ha2):
                    continue  # Skip identical vectors
                try:
                    _, _, disparity = procrustes(ha1.reshape(-1, 1), ha2.reshape(-1, 1))
                    procrustes_distances["halu_halu"].append({
                        "question": question,
                        "pair_indices": (i, j),
                        "distance": disparity,
    ...
    # Additionally, compute right_hallu distances
    # Iterate over each question and find pairs between real and halu answers
    unique_questions = latent_df['question'].unique()
    ...
    for question in unique_questions:
        ...
        for i in range(len(real_latents)):
            for j in range(len(halu_latents)):
                # Calculate ROUGE score
                rouge_score = scorer.score(real_texts[i], halu_texts[j])

                ra = np.array(real_latents[i])
                ha = np.array(halu_latents[j])
                if np.array_equal(ra, ha):
                    continue  # Skip identical vectors
                try:
                    _, _, disparity = procrustes(ra.reshape(-1, 1), ha.reshape(-1, 1))
                    procrustes_distances["right_halu"].append({
                        "question": question,
                        "pair_indices": (i, j),
                        "distance": disparity,
    ...
    # Reload the distances from the file (optional)
    with open('procrustes_distances.json', 'r') as f:
        procrustes_distances = json.load(f)

    # Assuming these plotting functions are defined
    ...
    # Optionally, perform additional analyses or visualizations
